...
Aplicando \eqref{tiempo_im} en \eqref{eq:accion}, tenemos
\begin{align}
...
Es por esto que las trayectorias clásicas, necesarias para calcular la integral de camino euclideano dada por la ecuación \eqref{eq:Isaddle} en la aproximación de punto estacionario, deben empezar y terminar en dicha región. Es decir, $x_i = x_f = x_+$. 
...
En la figura \ref{fig:soluciones} se encuentran ilustradas las tres trayectorias clásicas a considerar. La primera es la solución estática del falso vacío $x_{\text{FV}}\qty(\tau)$ (FV static en la figura \ref{fig:soluciones}) en la que la partícula permanece en el falso vacío para todo $\tau$,
\begin{equation}
x_{\text{FV}}\qty(\tau) = x_+,
\end{equation}
es decir, es la solución trivial. Como $V\qty(x_+) = 0$ y la solución es constante, $S[x_{\text{FV}}\qty(\tau)] = 0$ y su contribución a la integral de camino euclideana es simplemente
\begin{equation}
...
De igual manera, $\mathcal{E} = 0$. Esta condición debe cumplirse para las otras dos trayectorias clásicas.
...
Analizando el potencial invertido de la figura \ref{fig:potencial_invertido} notamos que ahora es posible encontrar trayectorias clásicas no triviales que atraviesen la barrera del potencial original. Consideremos una partícula que inicia su movimiento en $x_+$ lentamente. 
Si los tiempos inicial y final son $\tau = \pm \infty$ respectivamente, obtenemos el bounce \footnote{Mantendremos el nombre en su idioma original.}. 
